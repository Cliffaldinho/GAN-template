...
class Discriminator(nn.Module):
    def __init__(self,
                 ...
                 features=[64, 128, 256, 512, 1024, 512, 256],
    ...
    def forward(self, x):
        ...
        print("Initial input in discriminator")
        print(x.shape)
        print()
        x = self.initial(x)  # send it through the initial thing
        print("After sent through initial")
        print(x.shape)
        print()
        model_x_result = self.model(x)
        print("After sent through model")
        print(model_x_result.shape)
        print()
